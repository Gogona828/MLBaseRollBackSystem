...
擬似遅延中は、両者の画面下部に黒文字で **「擬似遅延発生中（100 ms）」** と表示します。サーバーの開始・終了通知と時計同期を使うため、FEP予測が使われただけでは表示しません。終了通知が欠落しても期限で消えます。通知の到着・描画に伴う数フレームの差はあります。この表示を使うにはサーバーも更新して再起動してください。
...
LAN設定時に同期コントローラーのGameObjectを有効化します。シーンで非アクティブでも動作し、終了待ちだけが有効になって次ラウンドへ進めなくなる問題を防ぎます。次ラウンドのフレーム番号は、両者が送信済みの先送り入力より後から開始します。
...
LANモードでは入力を既定で **3フレーム先送り**し、ローカル側も相手側も同じフレームで適用します（60Hzでは約50msの操作遅延）。通常の通信到着差だけで毎フレーム予測・訂正が起こることを抑えます。`NetworkInputSender` のInspectorの **LAN input buffer > Lan Input Buffer Frames** で0〜12に変更できます。両PCで同じ値にしてください。0では先送りなしになります。

受信キューをシミュレーション直前に処理し、FEP推論は毎フレーム継続します。Fight中に対象フレームの入力がまだ届かなかった場合だけ予測を適用します。ロールバックは完了済みフレームまでを再計算し、現在フレームは訂正後の状態から作った予測で一度だけ進めます。イントロ・K.O.・終了待機中は予測攻撃・移動を適用しません。
...
実施済み: 実UDPソケットによる双方向転送・遅延・開始通知・損失・枠衝突・ラウンド結果転送テスト、8〜10秒間隔と停止解除・順序保持・両者の準備待合せ・再通知の遅延開始・終了の両クライアント通知を含む計10件、異なる時計からの共通締切・古い開始通知の拒否テスト、3ホライズン×128入力（NaN含む）の元モデルおよびC#木評価の一致、プロジェクトのUnity参照DLLを使ったC#コンパイル、状態比較の許容差・HP・行動・KO等10ケース、遅延表示の期限切れと古い通知の拒否。
